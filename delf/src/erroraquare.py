...
class MoveOpenLoop():
    ...
    def __init__(self, plan = []):
        rospy.init_node('erroraquare', anonymous=False)
        ...
        self.omega = rospy.get_param('~ang_velocity', 0.4)
        self.jj=[]
        ...
        self.xx= np.zeros((0,20))
        self.yy= np.zeros((0,20))
    ...
    def move_straight_for_distance_using_odometry(self):
        ...
        for j in range(len(self.xx[:,0])):
            for i in range(20):
                targetgoal=(self.xx[j,i],self.yy[j,i])
                while self.euclidean_distance(self.start_xy,(self.x, self.y)) < self.euclidean_distance(targetgoal,self.start_xy):
                    (position, rotation) = self.get_odom()
                    self.x_error.append(position.x - targetgoal[0])
                    self.y_error.append(position.y - targetgoal[1])
    ...
    def rotate_for_angle_using_odometry(self, target_rotation):
        ...
        while abs((self.start_yaw-self.yaw)) < abs(target_rotation):
            self.vel_pub.publish(self.velocity)
            self.rate.sleep()
    ...
    def execute_plan(self, repeat = 1):
        ...
        for r in range(repeat):
            ...
            self.X_error.append(self.x_error)
            self.Y_error.append(self.y_error)
            t=self.t_p
        ...
        def getpoint(self):
            ...
            Y2 = np.linspace(3,0,20)
            X2 = np.linspace(2,2,20)
            X3 = np.linspace(-2,2,20)
            Y3 = np.linspace(3,3,20)
            X4 = np.linspace(-2,-2,20)
            Y4 = np.linspace(-3,3,20)
            X5 = np.linspace(2,-2,20)
            Y5 = np.linspace(-3,-3,20)

            self.xx=vstack((self.xx, X1,X2,X3,X4,X5))
            self.yy=vstack((self.yy, Y1,Y2,Y3,Y4,Y5))
...
if __name__ == '__main__':
    ...
    #plan = [(2,0,0),(2,3,0),(-2,3,90),(-2,-3,0),(2,-3,-90),(2,0,0)]
    plan = [(2,0,90),(2,3,90),(-2,3,90),(-2,-3,90),(2,-3,90),(2,0,-90)]
