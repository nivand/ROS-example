    ...
    def __init__(self):
        ...
        for i in range(4):
            ...
            while distance < goal_distance and not rospy.is_shutdown():
                ...
                distance = sqrt(pow((position.x - x_curr), 2) + pow((position.y - y_curr), 2))
                dist = sqrt(pow((position.x ), 2) + pow((position.y ), 2))
                error = sqrt(pow((position.x - x_goal), 2) + pow((position.y - y_goal), 2))
                print('---xcurr----','---position.x----- ',x_curr,position.x )
                print('---ycurr----','---position.y----- ',y_curr,position.y )
                print('---distance----',;'----dist---',distance,dist)
                print('---error----',error)
